...
void threadFunc(task task) {
    ...
    Matrix local_result(rows, task.cols);

    for (int local_i = 0; local_i < rows; ++local_i) {
        int global_i = task.start_row + local_i;
        for (int j = 0; j < task.cols; ++j) {
            float sum = 0.0f;
            for (int k = 0; k < task.common; ++k) {
                sum += task.first_matrix->getData()[global_i][k] * task.second_matrix->getData()[k][j];
            }
            local_result.getData()[local_i][j] = sum;
        }
    }

    for (int i_local = 0; i_local < rows; ++i_local) {
        for (int j = 0; j < task.cols; ++j) {
            task.result_matrix->getData()[task.start_row + i_local][j] = local_result.getData()[i_local][j];
        }
    }
}
...
Matrix Matrix::multiply(Matrix& first_matrix, Matrix& second_matrix) {
    ...
    int thread_num = std::min(std::thread::hardware_concurrency(), static_cast<unsigned int>(rows));
    int step = rows / thread_num;

    for (int i = 0; i < thread_num; ++i) {
        int start_row = i * step;
        int end_row = (i == thread_num - 1) ? rows : (start_row + step);
